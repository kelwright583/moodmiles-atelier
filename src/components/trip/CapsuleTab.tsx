    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            <div className="space-y-4 pt-2">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((c) => (
                    <button key={c} type="button" onClick={() => setCategory(c)} className={`px-3 py-1.5 rounded-full text-xs font-body transition-all ${category === c ? "bg-gold text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>{c}</button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="eyebrow text-muted-foreground mb-2 block">Style Tags</label>
                <div className="flex flex-wrap gap-2">
                  {styleTags.map((tag) => (
                    <button key={tag} type="button" onClick={() => toggleTag(tag)} className={`px-3 py-1 rounded-full text-xs font-body transition-all ${selectedTags.includes(tag) ? "bg-gold text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>{tag}</button>
                  ))}
                </div>
              </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((item) => (
            <div key={item.id} className="glass rounded-xl p-6 hover:glow-gold transition-all duration-300 group relative">
