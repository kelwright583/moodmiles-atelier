...
import { Calendar, ArrowRight, TrendingUp, Sparkles, X, ChevronRight, Crown, Plane, Users, Share2, Sun, CheckCircle2 } from "lucide-react";
      ...
      <main className="pt-24 md:pt-28 pb-16 md:pb-20 px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center gap-4">
              {profile?.avatar_url ? (
                <img
                  ...
                  alt=""
                  className="w-14 h-14 rounded-full object-cover border-2 border-gold/25"
                />
              ) : null}
              <div>
                <p className="eyebrow mb-2">My Suite</p>
                <h1 className="text-4xl md:text-6xl font-heading font-normal">
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-20">
            <h2 className="eyebrow mb-8">Upcoming Journeys</h2>
            {isLoading ? (
                ...
                {trips.map((trip) => {
                  ...
                  return (
                    <Link to={`/trip/${trip.id}`} key={trip.id}>
                      <div className={`group glass rounded-2xl overflow-hidden transition-all duration-500 cursor-pointer ${
                        isActive
                          ? "ring-1 ring-live-dim glow-live"
                          : isCompleted
                            ? "opacity-75 hover:opacity-100 hover:border-gold/30 hover:glow-gold"
                            : "hover:border-gold/30 hover:glow-gold"
                      }`}>
                        <div className="relative h-48 overflow-hidden bg-secondary">
                          ...
                          <div className="absolute top-4 right-4 flex items-center gap-2">
                            {trip.is_public && (
                              <span className="text-xs bg-live/15 text-live-text border border-live-dim rounded-full px-2 py-0.5 font-body">
                                Public
                              </span>
                            )}
                            {isActive ? (
                              <span className="text-xs tracking-[0.15em] uppercase font-body bg-live/15 text-live-text border border-live-dim px-2.5 py-1 rounded-full flex items-center gap-1.5 backdrop-blur-sm">
                                <span className="w-1.5 h-1.5 rounded-full bg-live text-live-text animate-pulse-dot" />
                                Travel Mode
                              </span>
                            ) : isCompleted ? (
                              <span className="text-xs tracking-[0.15em] uppercase font-body bg-secondary/80 text-muted-foreground border border-border px-2.5 py-1 rounded-full flex items-center gap-1.5 backdrop-blur-sm">
                          ...
                          {isActive && (
                            <div className="absolute bottom-3 left-4 right-4">
                              <div className="flex items-center gap-2">
                                <Sun size={14} className="text-live-text flex-shrink-0" />
                                <span className="text-sm font-heading text-white drop-shadow-md">
                                  {timing.label}
                                </span>
                                <span className="text-xs text-live-text/80 font-body ml-auto">
                                  {timing.daysLeft} day{timing.daysLeft === 1 ? "" : "s"} left
                                </span>
                              </div>
                              <div className="mt-2 h-1 rounded-full bg-white/10 overflow-hidden">
                                <div
                                  className="h-full rounded-full bg-live transition-all duration-1000"
                        ...
                        <div className="p-6">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <h3 className="text-2xl md:text-3xl font-heading font-normal">{trip.destination}</h3>
                              <p className="text-sm text-muted-foreground font-body">{trip.country}</p>
                            </div>
                            <ArrowRight size={18} className={`mt-1 opacity-0 group-hover:opacity-100 transition-opacity ${isActive ? "text-live-text" : "text-primary"}`} />
                          </div>
                          <div className="flex flex-col gap-1 text-xs font-body">
                            ...
                            <span className="text-muted-foreground flex items-center gap-1.5">
                              <Calendar size={12} className={isActive ? "text-live-text" : "text-primary"} />
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="mb-20">
            <div className="flex items-center gap-2 mb-8">
              ...
              <h2 className="eyebrow">Shared with Me</h2>
            </div>
          {sharedTrips.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {sharedTrips.map((trip: any) => (
                  <Link to={`/trip/${trip.id}`} key={trip.id}>
                    <div className="group glass rounded-2xl overflow-hidden hover:border-gold/30 hover:glow-gold transition-all duration-500 cursor-pointer border border-dashed border-primary/30">
                      <div className="relative h-48 overflow-hidden bg-secondary">
                        {trip.image_url ? (
                          <img
                            src={optimiseImageUrl(trip.image_url, 600)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              {previewTrending.map((t, i) => (
                <motion.div
                  ...
                  className="glass rounded-xl overflow-hidden hover:border-gold/30 hover:glow-gold transition-all duration-500 group/card"
                >
                  <button onClick={() => setShowTrendingFeed(true)} className="w-full text-left">
                    ...
                    <div className="p-5">
                      <p className="text-2xl font-heading font-normal">{t.city}</p>
                    </div>
                  </button>
                ...
                <div className="max-w-xl mx-auto space-y-5 px-4">
                  {trending.map((t, i) => (
                    ...
                    >
                      ...
                      <div className="p-5">
                        <h3 className="text-2xl font-heading font-normal mb-3">{t.city}</h3>
                        <button
